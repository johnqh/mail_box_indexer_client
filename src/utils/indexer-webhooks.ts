...
export class IndexerWebhookHelper {
   ...
   */
  async processEmailSent(
    emailData: WebhookEmailSent
  ): Promise<WebhookResponse> {
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/webhook/email-sent`,
      emailData
    );
   ...
   */
  async processRecipientLogin(
    loginData: WebhookRecipientLogin
  ): Promise<WebhookResponse> {
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/webhook/recipient-login`,
      loginData
    );
  ...
  async registerReferral(
    referralData: WebhookReferralRegistration
  ): Promise<WebhookResponse> {
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/points/register-referral`,
      referralData
    );
  ...
  async processRefereeLogin(
    ...
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/points/referee-login`,
      {
        walletAddress,
        signature,
        message,
   ...
   */
  async processLogin(
    loginData: WebhookLoginEvent
  ): Promise<WebhookLoginResult> {
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/webhook/login`,
      loginData
    );
  ...
  async processSolanaWebhook(transactions: unknown[]): Promise<{
    ...
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/solana/webhook`,
      transactions
    );
