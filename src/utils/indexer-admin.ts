...
export class IndexerAdminHelper {
  ...
  async createCampaign(
    ...
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/admin/campaigns/create`,
      {
        campaign,
        walletAddress,
        signature,
  ...
  async deactivateCampaign(
    ...
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/admin/campaigns/${encodeURIComponent(campaignId)}/deactivate`,
      {
        walletAddress,
        signature,
        message,
  ...
  async awardPoints(
    ...
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/admin/points/award`,
      {
        ...award,
        walletAddress,
        signature,
  ...
  async flagUser(
    ...
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/admin/points/flag-user`,
      {
        ...userFlag,
        walletAddress,
        signature,
  ...
  async getOverviewStats(adminSignature: string): Promise<AdminOverviewStats> {
    const response = await this.client.get(
      `${getIndexerBaseUrl(this.config)}/admin/stats/overview`,
      {
        headers: {
  ...
  async getFlaggedUsers(
    ...
    const response = await this.client.get(
      `${getIndexerBaseUrl(this.config)}/admin/users/flagged?${params}`,
      {
        headers: {
          'x-admin-signature': adminSignature,
        },
  ...
  async generateBulkCodes(
    ...
    const response = await this.client.post(
      `${getIndexerBaseUrl(this.config)}/admin/campaigns/bulk-codes`,
      {
        campaignId,
        count,
        walletAddress,
